...
const AudioManager = {
  ...
  async unlock() {
    if (this.unlocked) {
      console.log('[AudioManager] уже разблокирован, пропускаю');
      return;
    }
    /* БАГ-FIX: гонка — если unlock() вызван повторно пока первый ещё выполняется,
       второй должен выйти. Используем флаг _unlocking. */
    if (this._unlocking) {
      console.log('[AudioManager] unlock() уже выполняется, пропускаю');
      return;
    }
    this._unlocking = true;
    console.log('[AudioManager] unlock() начат (user gesture)');
    try {
      ...
      this.ctx = new AC();
      console.log(`[AudioManager] AudioContext создан, state="${this.ctx.state}"`);
      ...
      src.start(0);
      console.log('[AudioManager] беззвучный буфер воспроизведён (iOS unlock)');

      await this.ctx.resume();
      console.log(`[AudioManager] ctx.resume() выполнен, state="${this.ctx.state}"`);

      this.unlocked   = true;
      this._unlocking = false;
      console.log('[AudioManager] ✓ разблокирован! Начинаю загрузку звуков…');

      /* Загружаем все звуки в фоне через XHR */
      this._loadAll();
    } catch (e) {
      this._unlocking = false;
  ...
  _load(key, url) {
    ...
    xhr.onload = () => {
      if (xhr.status !== 200 && xhr.status !== 0) return; // 0 = file://
      ctx.decodeAudioData(
        xhr.response,
        buf  => {
          this.buffers[key] = buf;
          console.log(`[AudioManager] ✓ загружен: ${key}`);
        },
        err  => console.log(`[AudioManager] decode skip (${key}): синтез`)
      );
    };
    xhr.onerror = () => console.log(`[AudioManager] файл не найден (${key}): синтез`);
...
function pickRandom(segments) {
  ...
  /* БАГ-FIX: если пул редкости пуст — берём любой элемент И логируем предупреждение.
     В норме этого не должно происходить при корректных данных. */
  if (!pool.length) {
    console.warn(`[pickRandom] пул редкости "${rarity}" пуст, беру случайный элемент`);
...
const WheelRenderer = {
  ...
  _resize() {
    /* БАГ-FIX: devicePixelRatio > 2 убивает fps на старых iPhone — ограничиваем до 2 */
    const dpr  = Math.min(window.devicePixelRatio || 1, 2);
    const rect = this.canvas.getBoundingClientRect();
    const size = Math.round(Math.min(rect.width, rect.height));
    /* БАГ-FIX: если size === 0 (canvas скрыт) — не трогаем canvas, иначе ctx сломается */
    if (size === 0) return;
  ...
  draw(angle) {
    this.angle = angle;
    const ctx  = this.ctx;
    /* БАГ-FIX: тот же cap что в _resize, иначе координаты расползаются */
    const dpr  = Math.min(window.devicePixelRatio || 1, 2);
  ...
  highlightWinner(winIndex, angle) {
    ...
    ctx.fill();
    /* БАГ-FIX: shadowBlur/shadowColor устанавливаем ДО stroke, сбрасываем внутри save/restore */
    ctx.shadowColor = rc.glowColor;
    ctx.shadowBlur  = 20;
    ctx.strokeStyle = rc.textColor;
    ctx.lineWidth   = 3;
    ctx.stroke();
    /* shadowBlur автоматически сбрасывается через ctx.restore() ниже */
    ctx.restore();
  },
...
const ResultModal = {
  ...
  init() {
    ...
    /* ── Защита от null: если элементов нет — логируем и выходим ── */
    if (!this.el) {
      console.error('[ResultModal] КРИТИЧНО: #resultOverlay не найден в DOM!');
      return;
    }
    if (!this.closeBtn) {
      console.error('[ResultModal] КРИТИЧНО: #resultCloseBtn не найден в DOM!');
      return;
    }

    this.closeBtn.addEventListener('click', () => {
      console.log('[ResultModal] закрыт кнопкой');
      this.hide();
    });
    ...
    this.el.addEventListener('click', e => {
      if (e.target === this.el) {
        console.log('[ResultModal] закрыт кликом по фону');
        this.hide();
      }
    });
  ...
  show(item, type = 'weapon') {
    if (!this.el) {
      console.error('[ResultModal] show() вызван до init() или элемент не найден!');
      return;
    }

    const rarity = item.rarity;
    const rc     = RARITY[rarity];
    console.log(`[ResultModal] показываю: "${item.name}" (${rarity}, тип: ${type})`);
    ...
    if (item.image && imgWrap && img) {
      /* Есть путь к изображению (оружие) */
      imgWrap.style.display = '';
      img.style.display     = '';
      img.src               = '';          // сброс — важно для iOS кэша
      img.onerror = () => {
        console.log(`[ResultModal] PNG не найден (${item.image}), показываю эмодзи`);
      ...
      /* Устанавливаем src ПОСЛЕ onerror — гарантия срабатывания на iOS */
      img.src = item.image;
    } else if (imgWrap && img) {
      /* Усложнение — нет картинки */
      imgWrap.style.display = '';
      img.style.display     = 'none';
      const fb = document.createElement('div');
      fb.className   = 'result-img-fallback';
    ...
    /* ── Показываем оверлей ──
       ...
       гарантирует запуск CSS transition на iOS Safari.        */
    this.el.style.display = 'flex';
    void this.el.offsetHeight;               // reflow
    this.el.classList.add('visible');
    console.log('[ResultModal] оверлей показан, класс visible добавлен ✓');
  },

  hide() {
    if (!this.el) return;
    this.el.classList.remove('visible');
    /* Убираем display после transition */
    setTimeout(() => {
      if (!this.el.classList.contains('visible')) {
        this.el.style.display = '';
      }
    }, 320);
    console.log('[ResultModal] скрыт ✓');
  }
};
...
function flashScreen(rarity) {
  ...
  /* БАГ-FIX: null-guard — если оверлей не найден, не падаем */
  if (!el) return;
...
class WheelController {
  ...
  init() {
    ...
    /* БАГ-FIX: используем именованную функцию и храним ссылку,
       чтобы не добавлять дублирующие listeners при повторном init() */
    this._resizeHandler = () => {
  ...
  _spin() {
    if (this.engine.spinning) {
      console.log(`[WheelController:${this.opts.type}] _spin() пропущен — уже крутится`);
      return;
    }
    console.log(`[WheelController:${this.opts.type}] _spin() начат`);
  ...
  _onDone(finalAngle, item, index) {
    console.log(`[WheelController:${this.opts.type}] _onDone → "${item.name}" (${item.rarity}), index=${index}, angle=${finalAngle.toFixed(4)}`);

    /* Выключаем режим вращения — возвращаем полное качество */
    this.renderer.spinning = false;
    AudioManager.stopSpinLoop();

    /* Верификация: сектор под стрелкой должен совпадать с winIndex */
    const verified = this.renderer.getSectorAtPointer(finalAngle);
    if (verified === index) {
      console.log(`[WheelController:${this.opts.type}] ✓ верификация OK: сектор ${index} под стрелкой`);
    } else {
      console.warn(`[WheelController:${this.opts.type}] ⚠ рассинхрон: ожидался ${index} (${item.name}), под стрелкой ${verified} (${this.opts.segments[verified]?.name})`);
    }

    /* Отрисовка + подсветка победного сектора */
    this.renderer.draw(finalAngle);
    this.renderer.highlightWinner(index, finalAngle);

    /* Кольцо — цвет редкости */
    const ring = document.getElementById(this.opts.ringId);
    ...
    /* Частицы — небольшая задержка для эффекта */
    setTimeout(() => {
      console.log(`[WheelController:${this.opts.type}] запускаю частицы (${item.rarity})`);
      Particles.burst(item.rarity);
    }, 100);

    /* Звук по редкости и типу колеса */
    const soundDelay = item.rarity === 'legendary' ? 60 : 160;
    setTimeout(() => {
      console.log(`[WheelController:${this.opts.type}] воспроизвожу звук (${item.rarity})`);
    ...
    /* Карточка результата — показываем через 360мс после остановки */
    setTimeout(() => {
      console.log(`[WheelController:${this.opts.type}] показываю ResultModal`);
      try {
        ...
        console.log(`[WheelController:${this.opts.type}] ResultModal.show() выполнен ✓`);
      } catch(e) {
        console.error(`[WheelController:${this.opts.type}] ОШИБКА в ResultModal.show():`, e);
      }
    }, 360);

    /* Разблокировать кнопку через 1.8с */
    setTimeout(() => {
      const btn = document.getElementById(this.opts.spinBtnId);
      if (btn) btn.disabled = false;
      console.log(`[WheelController:${this.opts.type}] кнопка разблокирована`);
    }, 1800);
  }
...
const App = {
  init() {
    ...
    AudioManager.init();
    console.log('[App] AudioManager инициализирован, звук:', AudioManager.enabled ? 'вкл' : 'выкл');
